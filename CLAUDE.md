...
**`path` 不能以 `/mirrors/` 开头**（schema 直接报错）。这个板块的页面由 `[id].astro`
按 **`id`** 生成，`path` 在那儿是被忽略的；而「算不算本板块」却按 `path` 前缀判。
两者错位时页面照旧建在 `/mirrors/<id>/`，数据里却留下一个「说得出口、没人使用」的地址——
今天没有消费者读 `doc.path`，所以只错在数据上，将来有人按它生成链接就是 404。
...
判「所有源都缺」比的是**会走到这个变体的源数**（`variantMirrorTotal`，扣掉写了
`skip_variants` 的源）而不是全部源数：跳过该变体的源本来就不会报这个错，拿全量去比
永远差一个，模板问题会被误报成数据漏填。

还会报一种**声明了却没有任何模板引用**的变量（`unused-var`）：改模板时改漏的键
（拼错、改名、废弃）就藏在这里，不报的话它会一直躺在数据里谁也看不见。
参考集合取全部变体（含被 `skip_variants` 跳过的），所以「只被某个源跳过的变体用到」
不会被误报；`endpoint_var` 由框架消费，不算没人用。
...
| 复制按钮悬停字 `--on-ink-soft` | `#fff` / `#0b1f2b` | 在 `--ink-soft` 底上 7.85:1 / 10.60:1 |
| 错误 toast 字 `--on-danger` | `#fff` / `#3a0d07` | 在 `--danger` 底上 6.54:1 / 6.65:1 |

（最后两行是成对的「底色 + 文字」，写法是「亮值 / 暗值」；其余是亮色单值。）
...
82% 白（改成全白，当前项靠那 2px 底边区分，悬停靠下划线）、卡片上的源个数——
四处都在 3.4:1 上下。

**会翻转的 token 不能当底色配写死的白字。** `--color-inverse` 恒为 `#fff`（退化兜底的
代价，见上），而 `--ink-soft`、`--danger` 在暗色下是**浅色**——拿它们当底色时白字会掉到
1.59:1 / 2.54:1（Chromium 实测）。所以「落在某个底色上的文字」各自成对：
`--on-ink-soft`、`--on-danger`，底色翻、文字也跟着翻。**新增任何「X 底 + 文字」的组合前，
先在暗色下算一次**——亮色没问题不代表暗色没问题，这两处当初就是只在亮色下看着正常。
...
- 内容列上限 60rem（`max-w-content`），外壳上限 80rem（`max-w-page`）：
  **外壳变宽、正文本宽不变**，命令的折行行为与改造前逐字一致。
  正文列的上限由页面类型决定（`Base` 的 `wide`）：只有卡片网格（首页）放开到外壳宽，
  工具页一律 60rem——**包括没有侧栏的 `/dns/`**。别按「有没有侧栏」判宽度，
  那个页面的命令块会比别处宽出一整截。
